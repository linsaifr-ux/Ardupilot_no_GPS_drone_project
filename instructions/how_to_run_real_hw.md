...
> **Set expectations accordingly:** on real survey25 footage, this satellite-tile database gives ~286 m mean retrieval error against real drone imagery — confirmed as a domain-gap effect (season/lighting/content/perspective mismatch vs. the satellite source), not a bug and not a resolution problem (zoom-20 is this provider's confirmed ceiling, and a systematic sharpness-vs-error test found tile clarity barely correlates with error, r=-0.14 to -0.23). Rotating query frames to North-up before matching helps somewhat (~229 m, ~20% better — see `tools/extract_frames.py --rotate`) but doesn't close the gap. A same-domain database built from real footage of the same area gets ~8.6 m on the identical flight. Don't plan a contest flight around satellite-tile-database accuracy without a validation flight over the actual site first. Details: `instructions/vpe_jump_runaway_diagnosis.md` §14-19/§14-20/§14-23.
>
> **It also doesn't self-correct once off-track.** A live, position-aware SITL closed loop (real per-tick AnyLoc, not a scripted replay) confirmed across two independent runs that once the vehicle drifts a few hundred meters from wherever the reference database/imagery actually has coverage, retrieval doesn't recover — it settles into either an unbounded runaway or a bad-but-bounded drift, never pulling back toward truth. `EK3_GLITCH_RAD=50` and the VPE slew limiter (`control/vpe_slew.py`) both help with the *acute* symptom (sudden jumps), but neither fixes this — if the vehicle is ever meaningfully off the covered area, expect it to stay lost, not self-recover. Details: `instructions/vpe_jump_runaway_diagnosis.md` §14-22/§14-24.
...
The YOLO panel (top-left) always shows the live 30 fps camera frame (2026-08-15) with the latest `/yolo/detections` boxes drawn on top — boxes can trail a moving object by ~1 YOLO inference (~60 ms) but the video itself never freezes/steps. (The right-panel detection crops still use the older stamp-matched-frame approach, since each is a static per-detection snapshot where source accuracy matters more than smoothness.) If the detections feed goes quiet for 2 s the panel drops the boxes and shows a red `YOLO STALE (no boxes)` header — see `tools/README.md` for details.
...
Bitrate: 1 Mbps H.265, keyframe every 1 s. Override with `--bitrate N`. Latency is bounded (leaky queue after `appsrc` + `vbv-size` capped to `--bitrate`, 2026-07-07) so a momentary LTE stall drops stale frames instead of building up an ever-growing, non-recovering delay. If the connection itself drops (mode B, RTSP relay over LTE), the pipeline auto-reconnects with backoff (2026-07-09) instead of hanging — see `tools/README.md`.
...
| `ground_view_stream.py` YOLO panel header shows red `YOLO STALE (no boxes)` | No `/yolo/detections` message for >2 s — YOLO node died or hasn't started (it publishes every processed frame, even with zero detections, so silence means down) | Start/restart the detector; panel meanwhile shows the live camera feed without boxes |
| `ground_view_stream.py` YOLO boxes trail slightly behind moving objects (~1 inference, ~60 ms) | Expected, not a bug (2026-08-15) — the main panel always shows the live frame for smoothness, boxes are just the latest detection drawn on top | None needed; if the trail looks much larger than ~60 ms worth of motion, check YOLO's actual fps (should be ~17) rather than assume this is the cause |
...
| `ground_view_stream.py` logs `GLib-GIO-CRITICAL **: g_socket_set_timeout: assertion 'G_IS_SOCKET (socket)' failed` and the stream stops | `rtspclientsink` (mode B) lost the TCP connection to the relay (e.g. LTE drop) and can't recover on its own | Fixed 2026-07-09 — pipeline now auto-rebuilds on bus `ERROR`/`EOS` with backoff; pull latest code. Prints `[stream] Connection lost (...) — reconnecting …` when it happens |
| `ground_view_stream.py` OpenHD (mode C) stream goes black/dead after the OpenHD ground station reboots, and only restarting `ground_view_stream.py` itself brings it back (RTSP/mode B kept working fine the whole time, or died too, taking the process down) | The reconnect logic's pipeline *rebuild* step (not just the teardown) was unprotected — if `Gst.parse_launch`/`set_state(PLAYING)` raised right after the old pipeline was torn down (e.g. the ground station's port still refusing right after reboot), the exception killed the OpenHD leg's daemon thread outright (mode C) or fell through to `finally:` and killed the whole process (mode A/B) — either way nothing was left running to retry once the ground station came back | Fixed 2026-08-16 — rebuild is now wrapped in its own try/except in both `_reconnect()` and `_openhd_thread`'s `reconnect()`; a failed rebuild just logs and retries on the next loop iteration instead of dying. Pull latest code. Verify the composite itself isn't the problem first: check the local `recordings/ground_view_*.mkv` (tees the same frames mode C gets) — if that has real video, the drone side is fine and the fault is downstream (network/ground station) |
| `record_field.py` logs the same `GLib-GIO-CRITICAL g_socket_set_timeout` and **both the RTSP/OpenHD stream and local recording freeze together** (video.mkv/frame_times.csv stop advancing while imu.csv/telemetry.csv keep updating — the tell that it's this, not a camera dropout) | Same `rtspclientsink`/LTE-drop failure as above, but `record_field.py` ran all three sinks (`rec`, `stream`, `openhd`) with a blocking `appsrc` and no queue on one shared capture thread — the stuck network push froze the whole loop, not just the stream | Fixed 2026-08-11 — leaky queue after each network appsrc + bus-error auto-reconnect with backoff, same mechanism as `ground_view_stream.py`; local recording no longer shares fate with a stalled network sink. Pull latest code. If it recurs on old code: kill `record_field.py` + its `imu_logger.py` child and restart — `field_data_collection.sh`'s `trap ... EXIT` will also cycle MAVROS/relay, so expect a few seconds of dead GCS link during the restart |
