...
`rtspclientsink` handles RTP payloading internally — do **not** use `rtph265pay` before it.  
Use `h265parse` instead to ensure proper NAL framing:

```python
# BEFORE (UDP to ground station)
f'rtph265pay config-interval=-1 mtu=1200 ! '
f'udpsink host={ground_ip} port=5000 sync=false'

# AFTER (RTSP push to MediaMTX relay)
f'h265parse ! '
f'rtspclientsink location=rtsp://{server_ip}:8554/drone protocols=tcp'
```

> **Note:** feeding pre-payloaded RTP from `rtph265pay` into `rtspclientsink` causes  
> `gst_parse_error: could not link rtph265pay0 to rtspclientsink0` — the elements are incompatible.
