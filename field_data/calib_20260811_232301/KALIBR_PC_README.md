# Kalibr calibration — PC instructions for session calib_20260811_232301

> **Recorded + sanity-checked on the Jetson 2026-08-11.** Ready for Phase 3
> (measure the physical grid) and the PC-side Docker steps below. Not yet run
> through Kalibr. Mirrors `field_data/calib_20260723_001209/KALIBR_PC_README.md`,
> the completed calibration for the previous (IMX219) camera — this is the
> same procedure for the reverted AP-IMX900 body + new 4mm CS-mount lens.
>
> A second take (`calib_20260811_232621`, 86 s) was recorded the same night
> as a backup — kept un-processed in its own folder. Post-trim tag-detection
> stats are comparable (slightly higher median tags/frame, but ~40s less
> usable duration and less frame-edge coverage than this one), so this
> session was chosen as primary. Fall back to it only if this session's
> Kalibr run doesn't converge or fails acceptance.
...
- IMU: FC (Pixhawk 6C) raw IMU over MAVLink — not a Jetson-local IMU. Requested
  333 Hz, **332.0 Hz achieved** this session (`imu_rates.json`).
...
**This session was validated on the Jetson (OpenCV AprilTag-detector proxy for
Kalibr's own detector, `DICT_APRILTAG_36h11` / `markerBorderBits=2`):**

- 122.4 s video, 3656 frames (`frame_times.csv`), `frame_rotation_deg: 180` confirmed
  in `meta.json`, `purpose: calibration` confirmed.
- IMU: 40435 samples, mean interval 3.01 ms (≈332 Hz), **zero gaps > 14 ms** — clean,
  no dropouts.
- **First ~23 s is a defocused/blank prefix** (camera resting before pickup — sharpness
  ~2-8 and 0-1 tag detections flat until t≈22.3s, then jumps to sharpness 20-47 and
  22-34 tags/frame as the grid comes into frame). This is NOT the same value as
  `prepare_kalibr_input.py`'s script default (`--trim-head 12.5`, calibrated to the
  old IMX219 session) — **use `--trim-head 23` for this recording, verify visually
  if in doubt.**
- Tag-detection sweep on the post-trim content (~99 s, OpenCV proxy, 2 Hz sample):
  mean 12.8 tags/frame, median 6, **39.2 % of sampled frames ≥ 15 tags**, 40.5 % of
  frames 0 tags (handheld motion blur during repositioning — normal, Kalibr simply
  skips those), 41.8 % of frames with a tag touching within 12% of a frame edge
  (decent corner/edge coverage for distortion estimation).
...
| `meta.json` | `frame_rotation_deg: 180`, `purpose: calibration`, real `width`/`height`/`imu_requested_hz` |
| `prepare_kalibr_input.py` | copied from `tools/prepare_kalibr_input.py` (kept camera-agnostic there) — converts the above into Kalibr input (step 2) |
...
```bash
cd field_data/calib_20260811_232301
python3 prepare_kalibr_input.py --trim-head 23
```

`kalibr_input_full/` = all frames after the 23 s trim @30 fps (for the imu-camera
stage); `kalibr_input_4hz/` = every 8th frame (~3.75 Hz, for the intrinsics stage —
full rate would make it needlessly slow). Both contain the same `imu0.csv` (ns
timestamps).

**Before running Kalibr**, `imu.yaml`'s `update_rate` must be **332.0** (this
recording's real achieved IMU rate) — the script writes a 200.0 placeholder and does
NOT know the real rate; already confirmed above from `imu_rates.json`.
...
FOLDER=/absolute/path/to/calib_20260811_232301
...
Copy back into this session folder on the Jetson (`field_data/calib_20260811_232301/kalibr_output/`):
