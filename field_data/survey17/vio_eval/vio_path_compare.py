#!/usr/bin/env python3
"""survey17: combined path comparison — GPS truth vs OpenVINS segments."""
import csv, math
import numpy as np
import matplotlib
matplotlib.use("Agg")
...
t_e, xy_e, gt_e, alt_e = seg(f"{S}/vio_eval/vio_full.csv", 225, 430)
t_c, xy_c, gt_c, alt_c = seg(f"{S}/vio_eval/vio_cruise.csv", 448, 775)
...
ax.plot(c_al[:, 0], c_al[:, 1], color=C_CRUISE, lw=2.0,
        label="VIO cruise 448–775 s (aligned, scale as-is 0.43x)")
...
ax.set_title("survey17 — XY path: GPS truth vs OpenVINS (uncalibrated)",
...
ax2.plot(t_e - T0, alt_e, color=C_EARLY, lw=2.0, label="VIO alt — early run")
ax2.plot(t_c - T0, alt_c, color=C_CRUISE, lw=2.0, label="VIO alt — cruise run")
ax2.axvspan(400, 440, color="#e34948", alpha=0.08, lw=0)
ax2.annotate("climb 20→104 m:\nfilter overshoots 1.45x\nthen diverges", (401, 62),
             color="#a33", fontsize=9)
ax2.annotate("cruise alt drifts −13 m,\nrun diverges at 777 s", (530, 78),
