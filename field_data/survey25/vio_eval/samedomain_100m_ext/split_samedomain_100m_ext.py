#!/usr/bin/env python3
"""Split field_data/survey25_100m_agl_ext/frames.csv (AGL-restricted 95-105m extraction, WITH
--max-time-gap 2.0 so the t=306-318s loiter tail is covered) into interleaved DB (even orig_idx)
/ query (odd orig_idx) sets -- same methodology as samedomain_100m's split_samedomain_100m.py
(itself the same as Round 9's mixed-altitude same-domain split): even index -> database, odd
index -> held-out query.

Writes:
  field_data/survey25/vio_eval/samedomain_100m_ext/db_session/frames.csv   (db split, absolute paths)
  field_data/survey25/vio_eval/samedomain_100m_ext/db_frames_reference.csv (db split, for reference/audit)
  field_data/survey25/vio_eval/samedomain_100m_ext/query_frames.csv        (query split, orig_idx,path,...)
"""
import csv
import os

SRC_DIR = "/home/jetson/Ardupilot_no_GPS_drone_project/field_data/survey25_100m_agl_ext"
OUT_DIR = "/home/jetson/Ardupilot_no_GPS_drone_project/field_data/survey25/vio_eval/samedomain_100m_ext"
