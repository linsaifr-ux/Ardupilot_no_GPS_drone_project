...
ANCHOR_CSV = f"{V}/samedomain_100m/query_frames_mixed_timed.csv"
OUT_CSV = f"{V}/samedomain_100m/vio_full_samedomain_mixed_matched_fused.csv"
RESULT_JSON = f"{V}/samedomain_100m/mixed_matched_fusion_result.json"
...
# ---------------------------------------------------------------------------------------------
print("=" * 90)
print("STEP 2: building real anchor stream from samedomain_100m/query_frames_mixed_timed.csv "
      "(Round 9 mixed-altitude 50-100m AGL same-domain anchors, matched args for comparison)")
print("=" * 90)
rows = []
...
print("=" * 90)
row = report("Mixed-altitude same-domain anchor (50-100m, matched args, pull=0.9)", OUT_CSV)
print("\nfor reference:")
...
with open(RESULT_JSON, "w") as f:
    json.dump({
        ...
        "real_anchor_source": "anyloc_samedomain_result.json (Round 9) "
                              "(database_survey25_samedomain_vits14, "
                              "same-domain mixed 50-100m AGL) -- run with IDENTICAL "
                              "run_corrector args as the 100m-only run, for direct comparison",
