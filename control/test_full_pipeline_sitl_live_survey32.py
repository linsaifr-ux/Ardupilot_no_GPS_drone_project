...
default and this project's real anchor cadence) while SITL flies the survey32
AUTO mission:
  1. Read SITL's own TRUE position (SIMSTATE -- exactly as the existing harness).
  2. Find the REAL survey32 video frame whose RECORDED telemetry position is
  ...
  4. Run REAL AnyLoc (anyloc.localizer.AnyLocLocalizer, database
     anyloc/database_survey33_vits14 -- the same-domain database built from
     survey33's own footage, default feature mode) on that frame --
...
Between anchor ticks, position is dead-reckoned from vio_full_survey32.csv's
OWN frame-to-frame increments (survey32's own real, verified OpenVINS output --
not a gyro-predicted-tracking-enhanced proxy like survey25's version used), consumed as a queue paced by REAL elapsed SITL
...
pc's coordinate frame is local-ENU relative to survey32's own home
...
real survey32 frame just used / the matched AnyLoc database tile / both error
numbers as text) to field_data/survey32/vio_eval/sitl/postview/frame_%04d.png,
...
import test_full_pipeline_sitl_survey32 as base   # reuse mission-building + SITL plumbing (not modified)
from foundloc_corrector_survey32 import quat_yaw, build_argparser
...
VIO_CSV    = os.path.join(V, "vio_full_survey32.csv")
...
# ── survey32 frame/position index (for nearest-by-GPS lookup, not nearest-by-time) ──
...
class LiveCorrector:
    ...
    def __init__(self):
        ...
        # official config found 2026-07-27 (see field_data/survey32/vio_eval/README.md section 10):
        # default anchor_win=40 leaves the scale-estimation gate closed until t=156s (anchors
        # start at t=116s), a measured 27.7x divergence spike sat entirely inside that blind
        # spot. Halving to 20s cuts the divergence window's RMSE 5.3x with no cost elsewhere.
        # Deliberately NOT overriding anchor_win to 20 here (keeps the module default, 40.0),
        # and slew is NOT recommended for this live/closed-loop path either -- both were tested
        # 2026-07-27 (see field_data/survey32/vio_eval/README.md section 10 and memory
        # vpe-jump-runaway §14-31): anchor_win=20+slew is the best OFFLINE/batch config (cruise
        # RMSE 69.2m->27.3m, smooth), but in this LIVE closed loop specifically, slew made things
        # 4-5x WORSE regardless of anchor_win (mean localizer error 24-31m without slew vs.
        # 110-123m with it) -- rate-limiting the published correction lets the REAL vehicle drift
        # further before being pulled back, and since the next AnyLoc query is chosen by nearest-
        # REAL-position, more drift feeds worse-matching queries: a feedback loop that doesn't
        # exist in the offline evaluation (where truth is fixed, independent of what's published).
        # anchor_win=20 alone (no slew) was roughly neutral here (30.8m vs the default's 24.0m) --
        # not worth changing this path's default for. The offline and live paths now legitimately
        # use different tuned configs; that's a documented decision, not an oversight.
        self.args = a
...
def render_postview(idx, route_t, route_m, truth_hist, ekf_hist, pub_hist,
    ...
    ax_frame.set_title("real survey32 frame used (N-up rotated)")
    ax_frame.axis("off")
    ...
    fig.suptitle("survey32 LIVE position-aware SITL closed loop", fontsize=13)
    fig.tight_layout()
...
def run_flight(run_name: str):
    ...
    try:
        ...
        print(f"[live] flying survey32 mission ({run_name})…")
        route_start = time.monotonic()

        # Mission ends in LOITER_UNLIM (not LAND -- see base.mission_items_from_waypoints()),
        # matching this project's real flight profile (manual takeoff/landing). LOITER never
        # disarms on its own, so completion is the mission reaching its final item, not
        # ctrl.is_armed going false (which is what caused the ~607s disarm-hang in the
        # previous LAND-based version of this test -- see field_data/survey32/vio_eval/README.md
        # section 7). Also fixes a real bug from that version: the timeout here was a
        # hardcoded (315.0-170.0) literal copied from survey25's mission span, never
        # retargeted -- now uses base.CRUISE_SPAN_S (survey32's real ~81s cruise window).
        final_seq = len(items) - 1
        def reached_final_wp():
            if not route_start or (time.monotonic() - route_start) < 5:
                return False
            mc = mav.messages.get("MISSION_CURRENT")
            return mc is not None and mc.seq == final_seq
        pump(duration=None, cond=reached_final_wp, route_start=route_start,
             timeout=base.CRUISE_SPAN_S / max(SPEEDUP, 1) * 4 + 120)
