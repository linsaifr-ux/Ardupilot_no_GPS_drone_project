#!/usr/bin/env python3
"""
Closed-loop SITL test: does the real survey32 VIO+AnyLoc fusion pipeline's
...
Method
------
- A mission is built from survey32's own flown GPS track (telemetry.csv):
  HOME item, TAKEOFF to ~100 m AGL, a handful of cruise waypoints simplified
  from the real track (Douglas-Peucker), ending in LOITER_UNLIM at the last
  one (NOT LAND -- this project's real flights take off and land manually
  under pilot control; the pipeline's job is scoped to cruise only, see
  memory project-overview, confirmed 2026-07-27). Uploaded over MAVLink's
  MISSION_ITEM_INT protocol (no prior mission-upload code existed in this
  repo -- implemented here).
...
- At 20 Hz (matching the real commander's cadence) VISION_POSITION_ESTIMATE
  ...
  signal, replayed from field_data/survey32/vio_eval/vio_full_real_anchor_survey32.csv
  (the actual real-AnyLoc full-pipeline output for survey32, cross-session
  against the survey33-built database, verified this session) before this
...
Outputs: field_data/survey32/vio_eval/sitl/full_pipeline_sitl_<run>.json
         field_data/survey32/vio_eval/sitl/full_pipeline_sitl.png
"""
...
S          = os.path.join(ROOT, "field_data", "survey32")
V          = os.path.join(S, "vio_eval")
OUT_DIR    = os.path.join(V, "sitl")
os.makedirs(OUT_DIR, exist_ok=True)

FUSED_CSV  = os.path.join(V, "vio_full_real_anchor_survey32.csv")
TELEMETRY  = os.path.join(S, "telemetry.csv")
META       = os.path.join(S, "meta.json")

CRUISE_AGL = 100.0
CRUISE_SPAN_S = 197.2 - 116.0   # survey32's real AGL>=97m cruise window span
TICK_S     = 0.05          # 20 Hz vision feed (commander cadence)
SETP_HZ    = 4              # NAV_CONTROLLER_OUTPUT / MISSION_CURRENT poll rate share
HOLD_S     = 8.0            # settle time after reaching the final LOITER before ending capture
...
def build_mission():
    ...
    # cruise phase: survey32's real AGL>=97m cruise window is t=116.0-197.2s
    # video-relative (precisely measured, see field_data/survey32/vio_eval/README.md
    # section 1) -- much shorter/smaller than survey25's.
...
def mission_items_from_waypoints(wp_latlon, home_lat, home_lon, home_alt_msl):
    ...
    # end in LOITER at cruise altitude (not LAND) -- this project's real deployment does
    # takeoff and landing manually (pilot-controlled, GPS available for both), the
    # pipeline's job is scoped to cruise only (confirmed by Frank 2026-07-27, see memory
    # project-overview). An autonomous LAND item doesn't match how this will actually be
    # flown, and caused a real (unrelated-to-the-pipeline, unresolved) disarm hang in
    # earlier testing -- ending in LOITER_UNLIM sidesteps that entirely and is a more
    # honest model of the real flight profile: mission "completes" by holding position,
    # not by autonomously landing.
    last_lat, last_lon = wp_latlon[-1]
    items.append((F, mavutil.mavlink.MAV_CMD_NAV_LOITER_UNLIM, 0, 1,
                  0, 0, 0, 0, int(last_lat * 1e7), int(last_lon * 1e7), CRUISE_AGL))
    return items
...
def run_flight(run_name: str, fused_csv=None):
    ...
    try:
        ...
        print(f"[SITL-full] flying survey32 mission ({run_name} VPE)…")
        route_start = time.monotonic()
        # Mission ends in LOITER_UNLIM (not LAND -- see mission_items_from_waypoints()),
        # matching this project's real flight profile (manual takeoff/landing, pipeline
        # only covers cruise). LOITER never disarms on its own, so completion is detected
        # by the mission reaching its final item (seq == last item), not by ctrl.is_armed
        # going false. Held for a few seconds once reached to be sure it's a real arrival,
        # not a transient MISSION_CURRENT blip.
...
def plot():
    ...
    for col, run_name in enumerate(runs):
        ...
        ax.plot([p[0] for p in route_m], [p[1] for p in route_m],
                "k--", lw=1.5, label="mission route (survey32-derived)")
    ...
    fig.suptitle("survey32 full pipeline, AUTO-mode SITL closed loop", fontsize=13)
    fig.tight_layout()
...
def main():
    ...
    if args[:1] == ["run"] and len(args) == 2:
        ...
        # e.g. `run samedomain field_data/survey32/vio_eval/some_variant/vio_full_variant_fused.csv`
