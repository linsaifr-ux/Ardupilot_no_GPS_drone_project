...
| `mavlink_relay_client.py` | mTLS bridge between a local plain-TCP MAVLink endpoint (mavros's `gcs_url`, or Mission Planner) and Frank's PC's relay hub. Same script, run with `--role vehicle` (Jetson) or `--role controller` (MP machine) — see `streaming/mavlink_relay_setup.md`. |
