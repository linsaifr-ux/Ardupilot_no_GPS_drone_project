...
| `launch_camera.sh` | `usb_camera_node.py`: AP-IMX900 USB3 (MJPG), 1280×960 @ 30 fps → `/drone/camera/image_raw` (rgb8). Custom node, not the stock `v4l2_camera_node` — this camera only exposes raw YUYV at 640×480, and the ROS2 v4l2_camera package (0.6.2) has no MJPEG decode, so it silently negotiates down to 640×480 instead of erroring. OpenCV's V4L2 backend decodes MJPG transparently, so this node requests MJPG directly. Device path is auto-detected by USB descriptor name (usually resolves to `/dev/video0`, but doesn't assume it — the old IMX219 CSI camera can be connected at the same time without conflict); set `CAMERA_DEV` to override. |
...
| `../vio_vpe/launch_shadow_mode.sh` | **Separate system** (2026-08-14) — live VIO+VPE shadow-mode observer: same MAVROS/camera/hw_bridge stack, but runs OpenVINS VIO + SuperPoint/LightGlue VPE + a fusion filter live and **never publishes to the FC** (no `ardupilot_commander.py`, no vision-pose injection) — a passive watch-and-log addition to a normal manually/AUTO-flown flight, not a replacement for `launch_real_hw.sh`. Streams by default (RTSP relay + OpenHD simultaneously) and starts YOLO alongside. See `vio_vpe/README.md`. |
...
| `mavlink_relay_client.py` | mTLS bridge between a local plain-TCP MAVLink endpoint (mavros's `gcs_url`, or Mission Planner) and Frank's PC's relay hub. Same script, run with `--role vehicle` (Jetson) or `--role controller` (MP machine) — see `streaming/mavlink_relay_setup.md`. TCP keepalive since 2026-07-09 so a silent LTE/NAT drop reconnects in ~60 s instead of hanging forever. Since 2026-07-21 the vehicle role filters both directions: drops incoming `REQUEST_DATA_STREAM` (msg 66) so MP's periodic stream-rate requests can't collapse `imu_logger.py`'s high-rate RAW_IMU stream (200 Hz, 333 Hz field standard since 2026-07-23), and drops outgoing `RAW_IMU`/`ATTITUDE_QUATERNION` (27/31) so that same stream (~29 KB/s at 200 Hz, more at 333) doesn't saturate the LTE uplink shared with the video push (measured: 5–6 s stream lag, RTT 1.4 s until filtered). `dropped GCS/outbound msg id N` log lines are normal with MP connected. |
