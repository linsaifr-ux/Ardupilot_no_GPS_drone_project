...
>
> **Cross-session result degraded further on the current camera (2026-08-12):** the same cross-session recipe repeated for the reverted AP-IMX900 global-shutter camera (`anyloc/database_survey40_samedomain_vits14`, built from survey40 — a ~100m AGL map-build flight, ~49 min before the query flight survey42, same day/zone/camera) gave **~159 m mean retrieval error** (global whole-DB search), with match confidence scores (mean ~0.22-0.24) not separating good from bad matches at all — much worse than the 24-25 m 2026-07-27 result on the old IMX219 camera, and not close to the satellite-DB-beating margin seen before. Cause not yet root-caused (candidates: the ~49 min lighting/shadow gap being more consequential than the ~15 min 2026-07-27 gap, or something specific to this camera/lens/farmland scene) — treat this as a live open question, not a settled "same-domain beats satellite" guarantee, until re-tested with a same-day/shorter-gap mapping flight for this camera. Full downstream fusion consequence: with anchors this weak, EVERY tested fusion scheme (production anchor-chain, VO+score-gate, and a FoundLoc-style DBSCAN+gyro-lock corrector with real anchors) either matched or was beaten by simply running VO alone (~36.5 m mean) — see `field_data/survey42/vio_eval/foundloc_corrector_survey42.py` and memory `anyloc-samedomain-newcamera-test`.
...
```bash
# Zone-sized, zoom-20 (recommended — this is how the active database was built):
/home/jetson/venv/anyloc/bin/python3 anyloc/build_database.py --model vits14 \
    ...
    --agl-min 65 --agl-max 65 \
    --sat-zoom 20 --grid-radius-m 780 \
    --sat-path anyloc/database_zone_z20_vits14/satellite.jpg
# Full-radius circle around CENTER_LAT/LON (larger, mostly wasted — old default):
/home/jetson/venv/anyloc/bin/python3 anyloc/build_database.py --model vits14
```

Notes on the zoom-20 flags: `--sat-zoom 20` needs `--grid-radius-m 780` (shrinks the tile fetch to ±1560 m so the `MAX_TEX=16384` cap costs less resolution — effective ≈0.19 m/px vs 0.53 at the zoom-18 default) and an explicit `--sat-path` (otherwise the build silently reuses the existing zoom-18 `simulator/satellite_ground.jpg` mosaic). Create the symlink once:

```bash
ln -sfn database_zone_z20_vits14 anyloc/database
```
...
```bash
ln -sfn database_zone_z20_vits14 anyloc/database   # satellite zoom-20, zone-sized (active)
ln -sfn database_zone_vits14 anyloc/database   # satellite zoom-18, zone-sized (fallback)
...
| Database not found | Check symlink: `ls -la anyloc/database` → should point to `database_zone_z20_vits14` |
